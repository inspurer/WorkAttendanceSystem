...
csv_rd = pd.read_csv(path_feature_known_csv, header=None)
...
class   PunchcardUi(wx.Frame):
    def __init__(self,superion):
        ...
        wx.Button(parent=self,pos=(50,120),size=(80,80),label="全自动签到\n脸对准即可").Enable(False)

        self.ExitButton =  wx.Button(parent=self,pos=(50,320),size=(80,50),label='退出刷脸')

        # 封面图片
        self.image_cover = wx.Image(loading, wx.BITMAP_TYPE_ANY).Scale(600, 480)
        # 显示图片
        self.bmp = wx.StaticBitmap(parent=self, pos=(180,20), bitmap=wx.Bitmap(self.image_cover))

        self.Bind(wx.EVT_BUTTON,self.OnETButtonClicked,self.ExitButton)


    def OnETButtonClicked(self,event):
        # 释放摄像头
        cap.release()
        # 删除建立的窗口
        ...
        if len(dets) != 0:
            ...
            for i in range(len(features_known_arr)):
                ...
                if compare == "same":  # 找到了相似脸
                    ...
                    wx.MessageBox(message=name + "您好，您已成功签到", caption="温馨提示")
